...
function articleTemplateFor(evidence) {
  if (evidence.questionId === "q.econ.asia_divergence") {
    ...
    return {
      purpose: "Answer why India stayed poor while the rest of Asia got rich, in the voice of someone who knows development economics cold and refuses both the lazy take and the strident op-ed. The puzzle that drives the whole piece: around 1950 India was no poorer than China, South Korea, Taiwan or its neighbours, and on some counts a little richer, yet over the next seventy years they pulled far ahead. The honest answer this article argues, walked as a causal chain in order, is the 'integrated East Asian model': those economies first built human capital (health, schooling, and crucially women's work), then forced up investment and saving, then moved labour into manufacturing and rode that escalator into ever more sophisticated exports, and India under-did every link in that chain. But the article is scrupulously disciplined about cause: it states plainly WHAT the data shows (India lagged on all of these) while being honest that the data cannot prove WHICH factor mattered most, or that there was a single cause. Walk the evidence act by act: (1) the same starting line and the great divergence; (2) human capital first, ending on India's startling outlier, how few of its women are in paid work; (3) the investment engine of saving and capital formation; (4) the manufacturing escalator India skipped, jumping from farms to services; (5) where that left India, in productivity, capability and power; and (6) the other side, told fairly: India did crush extreme poverty, built a real services economy, and did all of it as a continuous democracy while the East Asian transformations were often coerced. Carry the argument forward section by section; never treat the charts as a disconnected list. This is a flagship: it must be the most complete, most rigorous and most readable thing a layperson can find on the Sino-Indian and India-versus-Asia divergence, while an economist could trust every caveat. Never narrate the article itself ('this chart shows', 'as we saw above'); write about India and Asia, and let each chart sit beside the prose that discusses it. Never use em-dashes.",
      requiredSections: planned.length
        ? [
            ...
            "A closing reader-question H2, 'How should you read these comparisons?', which is the methodology and caveats section and must be the last section. Name the sources plainly (the Maddison Project for the long-run GDP reconstruction; the World Bank for most modern indicators; the Penn World Table for productivity per hour; the Harvard Growth Lab's Atlas of Economic Complexity; the UNDP Human Development Index; the V-Dem electoral democracy index; ILO modelled estimates for labour and the World Bank's Poverty and Inequality Platform for poverty). State plainly: Maddison's pre-1950 figures are reconstructions with wide error bars, to be read as broad trajectory not precise yearly levels, and its cross-century comparisons rest on an assumed single price level; PPP is a modelled price adjustment, not a market exchange rate, and market-rate and PPP figures are never mixed here; labour-force and poverty figures are modelled or survey-based and differ in level from one another; Taiwan and the city-states (Singapore, Hong Kong) are partial comparators, with Taiwan absent from World Bank and UN data and the city-states' trade figures inflated by re-exports; China's World Bank manufacturing series begins only in 2004; and that several factors moving together across these charts is not proof that any one of them caused the divergence. Write it as a short, honest paragraph a reader can trust, not a reference list."
          ]
        : [
            "Weren't India and China equally poor in 1950?",
            "Did India just reform too late?",
            "What did East Asia build first?",
            "Why do so few Indian women work for pay?",
            "Where did the money to grow come from?",
            "Why did India skip the factory stage?",
            "What does all this add up to today?",
            "So what did India get right?",
