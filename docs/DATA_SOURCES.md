...
| MoSPI PLFS API | Labour force: LFPR, WPR, UR, worker-status distribution, informality conditions, earnings (regular/casual/self-employed) | https://api.mospi.gov.in/api/plfs/getData | Open, no auth. 8 indicator codes + 21 query dimensions (sex, sector, age, education, social category, religion, state, industry section, occupation/NCO, enterprise type/size, employee contract, weekly status). Full code dictionary in `data/manual/plfs-api-codes.json`. Gotchas: host needs legacy TLS renegotiation (see memory `plfs-api-tls-and-calendar-year`); `year_type` is only Agriculture Year / Calendar Year (no Financial-Year rounds — the API dropped 2017-18..2023-24 and now serves Calendar-Year 2022-2025); All India is `state_code=99`; responses paginate (default `limit=10`). |
